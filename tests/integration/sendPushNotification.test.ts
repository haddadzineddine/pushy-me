...
describe('Send Push Notification', () => {
  it('should send a push notification', async () => {
    ...
    const pushyMe = new PushyMe(mockPushyMeService, '9fad0b4e99bb3463ef3d3d');

    const result = await pushyMe.sendPushNotification({ message: 'Hello World' }, 'device-token');
